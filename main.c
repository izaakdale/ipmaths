...
void get_broadcast_address(char *ip_addr, char mask, char *output_buffer)
{
  ...
  inet_ntop(AF_INET, &broadcast_addr, output_buffer, ABC_IP_MAX_LEN);
}

void get_abcd_ip_format(unsigned int ip_address, char *output_buffer)
{
  inet_ntop(AF_INET, &ip_address, output_buffer, ABC_IP_MAX_LEN);
}
...
void get_network_id(char *ip_address, char mask, char *output_buffer)
{
  ...
  inet_ntop(AF_INET, &network_id, output_buffer, ABC_IP_MAX_LEN);
}
...
int main(int argc, char **argv)
{
  ...
  {
    printf("Testing Q1..\n");
    char ip_address[ABC_IP_MAX_LEN + 1],
        output_buffer[ABC_IP_MAX_LEN + 1];
    memset(ip_address, 0, ABC_IP_MAX_LEN + 1);
    memcpy(ip_address, "192.168.2.10", strlen("192.168.2.10"));
    ip_address[strlen(ip_address)] = '\0';
    char mask = 24;
    memset(output_buffer, 0, ABC_IP_MAX_LEN + 1);
  ...
  {
    char output_buffer[ABC_IP_MAX_LEN + 1];
    memset(output_buffer, 0, ABC_IP_MAX_LEN + 1);
...

  {
    ...
    printf("Testing Q6..\n");
    char network_id[ABC_IP_MAX_LEN + 1];
    strncpy(network_id, "192.168.0.0", strlen("192.168.0.0"));
    network_id[ABC_IP_MAX_LEN] = '\0';
